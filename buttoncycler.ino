...
unsigned char pid[] = {"lzmqu78zwrsjfmam"};//*********处替换成涂鸦IoT平台自己创建的产品的PID
...
unsigned char dp_process(unsigned char dpid,const unsigned char value[], unsigned short length)
{
  switch(dpid) {
    ...
    case DPID_WORK_MODE:    //21.模式
      ...
      switch(dp_enum_value){
        ...
        case 1: // colour mode
          break;
        case 2: // scene mode
          colorfill (strip.Color(30, 144, 255));
          break;
        case 3: // music mode
          colorfill (strip.Color(148, 0, 211));
          break;
      }
      //Status changes should be reported.
      my_device.mcu_dp_update(dpid, value, length);
    ...
    case DPID_BRIGHT_VALUE:   //22.亮度
      strip.setBrightness(value);
      my_device.mcu_dp_update(dpid, value, length);
      break;
      
    case DPID_COUNTDOWN:    //26.倒计时
      colorfill (strip.Color( 255, 0,  0));
      my_device.mcu_dp_update(dpid, value, length);
      break;
      
    case DPID_MUSIC_DATA:   //27.音乐律动  
        my_device.mcu_dp_update(dpid, value, length);
        colour_data_control(value, length);
      break;
    
    case DPID_DREAMLIGHT_SCENE_MODE:    //51.炫彩情景
    ...
    case DPID_LIGHTPIXEL_NUMBER_SET:    //53.长度设置
      my_device.mcu_dp_update(dpid, value, length);
      break;
